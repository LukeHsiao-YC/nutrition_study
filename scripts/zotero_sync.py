...
用法:
  python3 scripts/zotero_sync.py push              # 本地 → Zotero(預設只推深讀;--all 全推)
  python3 scripts/zotero_sync.py pull              # Zotero → 本地
  python3 scripts/zotero_sync.py pull --process    # pull 後自動用 Gemini 萃取+評讀
  python3 scripts/zotero_sync.py pull --process --push   # 再自動 commit+push(全自動)
  python3 scripts/zotero_sync.py sync              # 先 pull(可加 --process)再 push
"""
import json
import os
import re
...
def main():
    ...
    do_process = "--process" in args   # pull 後自動萃取+評讀(用 Gemini)
    do_push = "--push" in args         # 完成後 git commit+push

    def process_and_push():
        if do_process:
            import autopilot
            autopilot.process_all(do_ingest=False, do_review=True)
            if do_push:
                autopilot.git_push()
