...
3. 用 helper 安全寫回(**不要手改 YAML**,交給腳本以免格式錯亂):
   ...
   JSON
   ```
4. **自我測驗卡(主動回想)**:在 body 追加一個「## 自我測驗(主動回想)」區塊,
   放 3–5 題「先問後答」的卡片,涵蓋:研究問題、設計與樣本、主要結果(帶數字)、
   一個限制/偏誤、一個「這篇能不能推論到臨床/我的研究」的批判題。格式:
   ```
   ## 自我測驗(主動回想)
   1. **Q:** 這篇的主要研究問題與設計是什麼？
      <details><summary>作答後展開</summary>...</details>
   ```
   用 `<details>` 讓答案預設收合(先回想再對答案)。這是內容區塊,可直接寫入 body。
5. 回報:條列填了哪些、哪些留空(缺證據)、以及 RoB 的整體評級。**提醒使用者逐項檢查 ai_draft 後,把認可的內容填進對應的 `confirmed`,再把 `depth` 改成 `done`。**
   若使用者還想知道「這篇可不可信」,建議接著跑 **paper-review**(GRADE + spin 稽核)。
